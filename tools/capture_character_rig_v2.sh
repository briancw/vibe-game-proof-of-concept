...
# Capture the polygon-rig character lab (character-v2) through Godot's
# standard renderer, mirroring the other character capture scripts.
...
xvfb-run -a -s "-screen 0 1920x1080x24 -ac -nolisten tcp" \
	...
	--rendering-driver opengl3 --audio-driver Dummy res://character-v2/labs/rig_lab_v2.tscn -- --capture

mv "$user_data_dir/data/godot/app_userdata/Modern Interiors Tile Concept/character_rig_v2.png" \
	"$output_dir/character_rig_v2.png"
