...
# Produces an incrementing, renderer-generated image for visual comparison.
# Existing captures are never overwritten.

set -eu

project_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
output_dir="$project_dir/artifacts/screenshots/character_room"
...
while [ -e "$output_dir/character_room_$(printf '%03d' "$capture_index").png" ]; do
